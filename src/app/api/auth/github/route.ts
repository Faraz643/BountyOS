...
export async function GET(request: Request) {
  ...
  auth.searchParams.set("scope", "read:user user:email repo");
  auth.searchParams.set("state", state);
  const response = NextResponse.redirect(auth);
  response.cookies.set("oauth_state", state, { httpOnly: true, sameSite: "lax", secure: process.env.NODE_ENV === "production", maxAge: 600, path: "/" });
  return response;
}
