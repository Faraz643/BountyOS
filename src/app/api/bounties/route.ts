  ...
  try {
    ...
    await Promise.all(ranked.slice(0, 50).map(async x => {
      ...
      return db.bounty.upsert({
        ...
        update: { title: x.bounty.issue.title, description: x.bounty.issue.description, reward: x.bounty.reward.amount, currency: x.bounty.reward.currency, rewardVerified: verified, rewardConfidence: confidence, rewardSource: x.bounty.signals?.rewardSource || null, status: x.bounty.status, labels: x.bounty.labels, technologies: x.bounty.technologies, updatedAt: new Date(x.bounty.updatedAt) },
        create: { id: x.bounty.id, source: x.bounty.source, sourceUrl: x.bounty.sourceUrl, owner: x.bounty.repository.owner, repository: x.bounty.repository.name, issueNumber: x.bounty.issue.number, title: x.bounty.issue.title, description: x.bounty.issue.description, reward: x.bounty.reward.amount, currency: x.bounty.reward.currency, rewardVerified: verified, rewardConfidence: confidence, rewardSource: x.bounty.signals?.rewardSource || null, status: x.bounty.status, labels: x.bounty.labels, technologies: x.bounty.technologies, discoveredAt: new Date(x.bounty.discoveredAt), updatedAt: new Date(x.bounty.updatedAt) }
      });
    }));
    ...
    return NextResponse.json({
      ...
      ai: { provider: process.env.OLLAMA_ENABLED === "false" ? "disabled" : "ollama", model: process.env.OLLAMA_MODEL || "qwen2.5-coder:7b", analyzedCount: 0, analyses: {}, message: "AI analysis runs separately so discovery is not blocked by the local model." }
    });
  } catch (e) {
