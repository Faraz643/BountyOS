...
function detectTechnologies(text: string, labels: string[]): string[] {
  ...
  return [...new Set(TECHNOLOGIES.filter((technology) => {
    const needle = technology.toLowerCase(); const index = haystack.indexOf(needle);
    if (index < 0) return false; const before = haystack[index - 1] ?? " "; const after = haystack[index + needle.length] ?? " ";
...
function mapIssue(issue: any, owner: string, name: string, verification?: ReturnType<typeof verifyRewardEvidence>): Bounty {
  ...
  return {
    id: `github:${owner}/${name}#${issue.number}`, source: verification?.source === "algora" ? "algora" : "github", sourceUrl: issue.html_url,
    repository: { owner, name, url: `https://github.com/${owner}/${name}` }, issue: { number: issue.number, url: issue.html_url, title: issue.title, description },
    reward: reward ?? { amount: 0, currency: "USD" }, status: "open", labels, technologies: detectTechnologies(`${issue.title}\n${description}`, labels), discoveredAt: new Date().toISOString(), updatedAt: issue.updated_at,
    signals: { comments: Number(issue.comments ?? 0), reactions: Number(issue.reactions?.total_count ?? 0), assignees: Array.isArray(issue.assignees) ? issue.assignees.length : 0, hasLinkedPullRequest: Boolean(issue.pull_request?.html_url), createdAt: issue.created_at, rewardVerified: Boolean(verification?.verified), rewardConfidence: verification?.confidence ?? 0, rewardSource: verification?.source ?? "none" }
  };
}

export class GitHubBountySource implements BountySource {
  name = "github"; private token = process.env.GITHUB_TOKEN;
  private async request<T>(url: string): Promise<T> { const res = await fetch(url, { headers: { Accept: "application/vnd.github+json", ...(this.token ? { Authorization: `Bearer ${this.token}` } : {}) }, next: { revalidate: 300 } }); if (!res.ok) throw new Error(`GitHub API error ${res.status}: ${res.statusText}`); return res.json(); }
  private async comments(owner: string, repo: string, issueNumber: number): Promise<string[]> { const data = await this.request<any[]>(`https://api.github.com/repos/${owner}/${repo}/issues/${issueNumber}/comments?per_page=100`); return data.map((x) => String(x.body ?? "")).filter(Boolean); }
  ...
  async search(filters: BountySearchFilters = {}): Promise<Bounty[]> {
    ...
    const unique = new Map<string, any>(); for (const issue of data.items) unique.set(issue.html_url, issue);
    const enriched = await Promise.all([...unique.values()].map(async (issue) => { const parts = issue.repository_url.split("/"); return this.enrich(issue, parts.at(-2)!, parts.at(-1)!); }));
    return enriched.filter((b) => !filters.paidOnly || b.reward.amount > 0).filter((b) => !filters.verifiedOnly || Boolean(b.signals?.rewardVerified)).filter((b) => !filters.minReward || b.reward.amount >= filters.minReward).filter((b) => !filters.technologies?.length || filters.technologies.some((t) => b.technologies.map((x) => x.toLowerCase()).includes(t.toLowerCase())));
  }
  async getBounty(id: string): Promise<Bounty | null> { const match = id.match(/^github:(.+)#(\d+)$/); if (!match) return null; const [owner, repo] = match[1].split("/"); const issue = await this.request<any>(`https://api.github.com/repos/${owner}/${repo}/issues/${match[2]}`); const bounty = await this.enrich(issue, owner, repo); return { ...bounty, status: issue.state === "open" ? "open" : "completed" }; }
