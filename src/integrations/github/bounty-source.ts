...
const MAX_CANDIDATES_TO_VERIFY = Number(process.env.BOUNTY_MAX_CANDIDATES || 12);
...
export class GitHubBountySource implements BountySource{
  ...
  private async inspect(issue:any,owner:string,name:string):Promise<Bounty|null>{
    ...
    const strict={eligible:true,reasons:[],issueAgeDays,inactivityDays,hasLinkedPullRequest:false,hasOpenPullRequest:false,hasAnyPullRequest:false};
  ...
  async search(filters:BountySearchFilters={}):Promise<Bounty[]>{
    ...
    const issues=(response.items??[]).slice(0,MAX_CANDIDATES_TO_VERIFY);
    const eligible:Bounty[]=[];
    for(let i=0;i<issues.length;i+=2){
      const batch=issues.slice(i,i+2);
      const results=await Promise.all(batch.map(async issue=>{const parts=String(issue.repository_url).split("/");return this.inspect(issue,parts.at(-2)!,parts.at(-1)!);}));
  ...
  async getBounty(id:string):Promise<Bounty|null>{const m=id.match(/^github:(.+)#(\d+)$/);if(!m)return null;const[owner,repo]=m[1].split("/");const issue=await this.request<any>(`https://api.github.com/repos/${owner}/${repo}/issues/${m[2]}`);return this.inspect(issue,owner,repo);}
